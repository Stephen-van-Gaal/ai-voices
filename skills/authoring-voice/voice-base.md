...
- **Size the document to its reader.** Before writing anything durable — a spec, a definition, a design record, a brief — name who reads it and what they must decide. Cut what does not serve that decision; of what stays, give the most room to what costs the reader most to get wrong. State the current conclusion rather than a play-by-play of how you reached it. Keep a failed query, corrected count or abandoned option when it changes what the reader can trust, explains why the evidence cannot support a claim, or prevents the same mistake. State its effect, not its chronology. Being precise is not a reason to be long.
...
- **Settle the shape before drafting.** Name the purpose and the reader, the through-line, and the section order. Say what each section carries. Describe any table you mean to build: its columns and what each holds. A figure takes a design conversation of its own. Use word targets where they help allocate attention, not as acceptance criteria. Ask for outline approval when the user requested it, when the shape makes a consequential choice, or when drafting would cross the agreed scope. Length alone does not create an approval gate.
